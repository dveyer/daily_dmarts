...
class Fmc_sales(propMap: Map[String, String], sparkMap: Map[String, String], pdate: String, tools: Tools) extends SparkBase(propMap, sparkMap) {
  ...
  var status = true
  val fmc_path = "/dmp/daily_stg/medallia/fmc/"
  ...
  def RunJob():Boolean= {
    if (spark != null)
      try {
        ...
        spark.read.parquet("/mnt/gluster-storage/etl/download/suz/fmc_gsm/*/").createOrReplaceTempView("fmc_gsm")
        ...
        val df1 = spark.sql(s"""
          ...
          where a.order_status_id  in (414,14) and a.pre_order_status_id is null
          and a.fmc_type_id in ('2','4')
          ...
          and t.order_status_date < '2018-05-28 17:00'
          ) q
          left join fmc_gsm g
          on q.id = g.fmc_order_id
          and g.sign_main_gsm = '1'
          and (g.expiration_date is null or g.expiration_date > current_date)
          where q.rn= 1
          and ((q.fmc_type_id in ('2') and q.prev_fmc_req_type is null)
          or (q.fmc_type_id = '4' and (q.prev_fmc_req_type is null or q.prev_fmc_req_type in ('3')))
          or (q.fmc_type_id = q.prev_fmc_req_type and q.prev_req_status = 4))

        """
        ).write.parquet(fmc_path + "28052018") //tools.patternToDate(P_DATE,"ddMMyyyy"))
